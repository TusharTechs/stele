...
export async function distillLessons(
  ...
  const prompt = [
    ...
    "  - Every rule is phrased as the thing TO DO, even one that exists to prevent a failure.",
    "    'Do not center the subject identically in each frame' is fine; so is 'Vary framing across shots'.",
    "  - kind says why the rule exists, not how it is worded: knownFailure to prevent a failure seen,",
    "    successfulPattern to keep something that worked, styleAnchor to lock a look, constraint for a",
    "    hard requirement.",
